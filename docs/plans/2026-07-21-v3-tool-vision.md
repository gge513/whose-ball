...
> **Step:** SESSION 1 IN PROGRESS (started 2026-07-21, same session — George's call to build now: v3 is separate from cohort work, momentum wins; Week 2 comms still owed by Sun Jul 26 17:00 ET, tracked in the project memory). **Session 1 structure locked, all four George's calls:** (1) real workspace model scoped down — `workspaces` + `workspace_members`, `workspace_id` on projects, cohort migrated as workspace #1, no roles yet; (2) vision renders on BOTH /me and momentum; (3) **project goals follow the Foundation goals-canon DNA** (read-only read of `~/the-foundation/goals/2026.md`, 2026-07-21): goals are plural per project, each = statement + key result gradeable by a stranger + owner + timeline, with a **dropped/decided-not-to-do state recorded openly** (the canon's "Decided: closed" rows) — NOT a grown Define field, and NOT a task list (tasks are work; goals are outcomes with measures); (4) **workspace #2 = the Foundation team, chosen out loud** — whose-ball stays an OUTSIDE tool the Foundation team uses; it does not fold into the Foundation OS (the separation line is crossed for membership, not for plumbing). **SESSION 1 DONE (2026-07-21, `700ec4b`, live-verified):** ~~schema~~ → ~~migration 0007~~ (hand-edited live-data-safe: seed ws #1 → add column nullable → backfill → NOT NULL → enroll all 31 users; the generated NOT-NULL-add would have failed on live rows — applied to prod via the neon client, VERIFY: 1 ws / 0 orphan projects / 31 members = 31 users / goal_met in enum) → ~~scope queries~~ (rosters + project listings through `lib/workspace.ts`; single-workspace era isolated in one module) → ~~vision renders~~ (set/revise on momentum, one-liner on /me) → ~~goals on the project page~~ (add + grade met/dropped; met emits `goal_met` to the feed; dropped renders "decided: closed", struck through, never deleted) → ~~build/verify/deploy~~ (all 8 routes 200 on prod). **RESUME AT SESSION 2 (the landing page tells the thesis) — or Week 2 comms first (merge Sun Jul 26 17:00 ET), George's call at next session start.** Not yet done, noted honestly: events/momentum feed is not workspace-scoped (no behavior difference in the single-workspace era; scope it when workspace #2 arrives), and the Foundation workspace itself is NOT created yet — that happens deliberately, with George naming its vision, likely alongside Session 4's invites.
