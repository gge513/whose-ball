...
> **Step:** SESSION D IN PROGRESS (structure locked 2026-07-13, all four on recommendation): (1) deploy FIRST, then features land on prod; (2) momentum feed + weekly narrative run on app events (tasks done, stage advances, reviews filed, blockers raised/cleared), GitHub merged-PR ingest designed-in but stubbed until real submission branches exist; (3) journey header = current phase + live countdown + YOUR standing, on every page; full spine only on the momentum page; (4) README thesis-led (motivation thesis → six decisions as feature tour → setup/architecture/URL/limitations tail). Resume at the first unfinished block: ~~deploy~~ → ~~momentum page~~ → ~~journey header~~ → ~~Assists + Possession view~~ → ~~Rally~~ → **push to prod + verify Rally/Assists live, then Friday-AM trigger check** (whistle-with-causes in scope only if Rally+Assists verified on prod by Fri Jul 17 AM) → weekly narrative (in the ratified B/B/A voice) → README+AGENTS.md (DFW epigraph per voice frame) → 10-user smoke. Spec for the new blocks: the Second harvest section below + `docs/brainstorms/2026-07-15-evernote-pm-communication-ore.md`. (Session D part 1 closed 2026-07-13; COURSE-LOG Entry 005 filed. Extras shipped: extensive 5-layer test all green; the gate referee `66489c2` — locked advance whistles with escalating calls + live N-to-answer counter, server action untouched.)
>
